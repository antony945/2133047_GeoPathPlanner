...
*   **Scalability:** Individual services (e.g., the `Routing Manager`) can be scaled independently to handle high computational loads without affecting other parts of the system.
...
*   **Technological Flexibility:** Each service can be implemented using the most appropriate technology stack. We leverage React for the frontend, Go for high-performance computing in the `Routing Manager`, and NestJS for the `User Management` service.
...
*   **Responsibilities:**
    ...
    *   Executing pathfinding algorithms (e.g., RRT, RRT*).
    *   Considering waypoints, obstacles, and other constraints during calculation.
    *   Publishing the final computed route to another Kafka topic for the API Gateway to retrieve.

## 4. Communication Patterns

The system employs a hybrid communication model to ensure both responsiveness and scalability.

*   **Synchronous (REST):** For immediate, request-response interactions like user login, registration, or fetching route history, the Frontend communicates with the API Gateway using standard RESTful API calls over HTTP.
...
2.  **API Gateway Receives Request:** The Frontend sends a `POST` request to the `/route/compute` endpoint on the **API Gateway**.
3.  **Job Publishing:** The API Gateway validates the request and publishes a message containing the routing parameters (waypoints, obstacles) to a Kafka topic (e.g., `routing_requests`).
4.  **Asynchronous Processing:** The **Routing Manager**, which is subscribed to the `routing_requests` topic, consumes the message and begins the pathfinding calculation. During this time, the **API Gateway** waits for the result from Kafka, and the **Frontend** displays a loading screen to the user.
...
6.  **Result Consumption:** The API Gateway listens to the `routing_results` topic. Upon receiving the result corresponding to the user's request, it forwards the data to the Frontend.
...
*   The **Frontend** sends all routing requests to the **API Gateway** (Python/FastAPI).
*   The **API Gateway** validates JWT tokens for authenticated requests (tokens issued by the **User Management** service).
*   For routing requests, the **API Gateway** publishes a message to a **Kafka** topic.
*   The **Routing Manager** (Go) consumes from this Kafka topic, computes the route, and publishes the result to another Kafka topic.
*   The **API Gateway** consumes the result from Kafka and sends it back to the **Frontend**.
*   The **API Gateway** also persists and retrieves route history from its own **Database**.
