...
func TestRTreeStorage_NearestPoint(t *testing.T) {
  ...
  w_close2dbuthigh := models.MustNewWaypoint(6, 40.41521155294711, -3.709898376035113, high)
	...
	tests := []struct {
		...
		{ name: "RTREE - Find NearestPoint to external point in presence of a 2d but not 3d nearest", w_list: w_list, c_list: c_list, p: w, want: w_list[2], wantErr: false, },
	}
...
func TestRTreeStorage_KNearestPoints(t *testing.T) {
	...
	tests := []struct {
		...
		{ name: "RTREE - KNN - external point - k=0", w_list: w_list, c_list: c_list, p: w, k: 0, want: []*models.Waypoint{}, wantErr: false },
		{ name: "RTREE - KNN - external point - k=1", w_list: w_list, c_list: c_list, p: w2, k: 1, want: []*models.Waypoint{w_list[0]}, wantErr: false },
		{ name: "RTREE - KNN - external point - k=3", w_list: w_list, c_list: c_list, p: w2, k: 3, want: []*models.Waypoint{w_list[0], w_list[3], w_list[1]}, wantErr: false },
		{ name: "RTREE - KNN - external point - k>#wps", w_list: w_list, c_list: c_list, p: w2, k: 7, want: []*models.Waypoint{w_list[0], w_list[3], w_list[1], w_list[2]}, wantErr: false },
		{ name: "RTREE - KNN - internal point - k=1", w_list: w_list, c_list: c_list, p: w_list[0], k: 1, want: []*models.Waypoint{w_list[0]}, wantErr: false },
		{ name: "RTREE - KNN - internal point - k=2", w_list: w_list, c_list: c_list, p: w_list[0], k: 2, want: []*models.Waypoint{w_list[0], w_list[3]}, wantErr: false },
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			m, err := storage.NewRTreeStorage(tt.w_list, tt.c_list)
      ...
      // TODO: update the condition below to compare got with tt.want.
      if !assert.ElementsMatch(dummyt{}, got, tt.want) {
